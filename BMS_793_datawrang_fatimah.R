...
fatim.data.postme %>% group_by(id) %>% mutate(shft.amp = amp-amp[time==0])
...
dg.data.prop <- dg.data
dg.data.prop[,-c(1,6,7)] <- sweep(dg.data[,-c(1,6,7)],MARGIN=2,temp,"/")

dg.data.tax1 <- aggregate(.~tax1,dg.data.prop[,-c(1,7)],sum)
dg.data.tax2 <- aggregate(.~tax2,dg.data.prop[,-c(1,6)],sum)
